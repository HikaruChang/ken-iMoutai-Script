    ...
    try:
        ...
        for item in reservations:
            ...
            if reservation_time == today:
                ...
                session_name = (
                    f"[{item.get('sessionName', '')}]"
                    if item.get("sessionName")
                    else ""
                )
...
if __name__ == "__main__":
    ...
    logging.info("--------------------------")
    logging.info(
        "💬 申购成功后将以短信形式通知您，请您在申购成功次日18:00前选择支付方式，并根据提示完成支付。"
    )
    for user in users:
        try:
            logging.info("--------------------------")
            ...
            check_reservation_result(
                user["TOKEN"], user["DEVICE_ID"], user["MT_VERSION"]
            )
        except Exception as e:
            logging.error(
